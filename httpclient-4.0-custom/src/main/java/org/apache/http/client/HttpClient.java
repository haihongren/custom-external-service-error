...
import com.nr.agent.instrumentation.httpclient40.NRURLUtilities;
...
public abstract class HttpClient {
    ...
    @NewField
    private boolean httpStatusMetricProcessed = false;
    ...
    public HttpResponse execute(HttpUriRequest request) throws Exception {
        ...
        if (!httpStatusMetricProcessed){
            int responseCode=response.getStatusLine().getStatusCode();
            NRURLUtilities.reportConnectionMetrics(request.getURI(), responseCode);
            httpStatusMetricProcessed =true;
        }
    ...
    public HttpResponse execute(HttpHost target, HttpRequest request) throws Exception {
        ...
        if (!httpStatusMetricProcessed){
        int responseCode=response.getStatusLine().getStatusCode();
        NRURLUtilities.reportConnectionMetrics(actualURI, responseCode);
        httpStatusMetricProcessed =true;
        }
        return response;
    }
    ...
    public <T, R extends T> T execute(HttpUriRequest request, ResponseHandler<R> responseHandler)
            throws Exception {
        responseHandler = new WrappedResponseHandler<>(request.getURI(), responseHandler, httpStatusMetricProcessed);
        httpStatusMetricProcessed =true;
        doOutboundCAT(request);
        T response;
        try {
    ...
    public <T, R extends T> T execute(HttpHost target, HttpRequest request, ResponseHandler<R> responseHandler)
            throws Exception {
        URI actualURI = getUri(target, request);
        responseHandler = new WrappedResponseHandler<>(actualURI, responseHandler, httpStatusMetricProcessed);
        httpStatusMetricProcessed =true;
        doOutboundCAT(request);
        T response;
        try {
    ...
    public static class WrappedResponseHandler<T> implements ResponseHandler<T> {
        ...
        private final boolean transactionMetricProcessed;
        public WrappedResponseHandler(URI uri, ResponseHandler<T> originalResponseHandler,boolean transactionMetricProcessed) {
            this.uri = uri;
            this.originalResponseHandler = originalResponseHandler;
            this.transactionMetricProcessed = transactionMetricProcessed;
        }

        @Override
        public T handleResponse(HttpResponse response) throws ClientProtocolException, IOException {
            try {
                processResponse(uri, response,transactionMetricProcessed);
            } catch (Throwable t) {
        ...
        private static void processResponse(URI requestURI, HttpResponse response, boolean transactionMetricProcessed) {
            InboundWrapper inboundCatWrapper = new InboundWrapper(response);
            NewRelic.getAgent().getTracedMethod().reportAsExternal(HttpParameters
                    .library(LIBRARY)
                    .uri(requestURI)
                    ...
                    .status(response.getStatusLine().getStatusCode(), response.getStatusLine().getReasonPhrase())
                    .build());
            if (!transactionMetricProcessed){
                int responseCode=response.getStatusLine().getStatusCode();
                NRURLUtilities.reportConnectionMetrics(requestURI, responseCode);
            }
        }
